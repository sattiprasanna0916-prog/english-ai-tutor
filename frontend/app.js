...
const speechStats = analyzeSpeech(lastAnswer, duration);
  ...
  if (speechStats.fillerCount > 3) {
    feedback.push(`⚠️ You used ${speechStats.fillerCount} filler words`);
  }

  feedback.push(`🗣️ Speed: ${speechStats.wpm} WPM (${speechStats.pace})`);

  // append to feedback UI
...
function highlightWeakSkill(skill){
  ...
  document.getElementById("fluencyBar").parentElement.classList.remove("weak");
  document.getElementById("grammarBar").parentElement.classList.remove("weak");
  document.getElementById("accuracyBar").parentElement.classList.remove("weak");
